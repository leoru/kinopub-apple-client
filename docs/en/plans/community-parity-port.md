...
- [ ] **Device settings** pane replaces the `@State` demo in `PlaceholderSettingsPanes.swift`
...
- [ ] **Devices** list + remove: `DeviceService.listDevices()` / `removeDevice(id:)` are already
...
- [ ] **Storage**: report what's actually introspectable today — `RowSnapshotStore` disk snapshots,
...
- [ ] **Sections** pane: persisted show/hide store + toggle screen (§8). Scope this pane's *first
...
- [ ] Feature-gate every pane that stays a stub instead of shipping fake toggles

## 8. Section visibility

- [ ] Persisted show/hide for catalog sections (their `SectionVisibilityStore`, `UserDefaults`)
- [ ] Forced-visible set: Movies, Series, Collections
- [ ] Settings → Sections toggles, grouped Library / Other
- [ ] Check against the `TabViewCustomization` ceiling noted in [local-caching](local-caching.md)
      before designing sidebar-side customization
