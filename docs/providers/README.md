...
| Kinopoisk proxy (`kpapp.link`) | **[kinopoisk-proxy.md](kinopoisk-proxy.md)** | **yes — offline pipeline + live worker** | Four endpoints, no auth, Kinopoisk id only: facts, reviews, staff, images. `fetch_kinopoisk_proxy.py` (lazy, per-title) wires all four into the record; the worker's live `/v1/title` currently wires `facts` only, with `staff`-based photos/reviews/images as the natural next step. Gaps named in the sheet — reviews are page 1 of N, `BLOOPER` facts and all non-actor crew are dropped |
