...
> 🔎 **Живой ответ — что на самом деле в `settings`.** Снято с PWA 2026-08-16; вендор здесь
> оставил комментарий вместо содержимого.
>
> ```json
> "settings": {
>   "supportSsl":     { "value": 1, "label": "Использовать SSL" },
>   "supportHevc":    { "value": 0, "label": "Поддержка HEVC" },
>   "supportHdr":     { "value": 0, "label": "Поддержка HDR" },
>   "support4k":      { "value": 0, "label": "Поддержка 4K" },
>   "mixedPlaylist":  { "value": 0, "label": "Смешанный плейлист" },
>   "serverLocation": { "type": "list", "label": "Переключиться на сервер",
>                       "value": [ { "id": 1, "label": "Нидерланды", "description": "", "selected": 1 },
>                                  { "id": 3, "label": "Россия",     "description": "", "selected": 0 } ] },
>   "streamingType":  { "type": "list", "label": "Тип потока",
>                       "value": [ { "id": 1, "label": "HTTP", "description": "HTTP псевдо стриминг", "selected": 0 },
>                                  { "id": 2, "label": "HLS",  "description": "", "selected": 0 },
>                                  { "id": 3, "label": "HLS2", "description": "", "selected": 0 },
>                                  { "id": 4, "label": "HLS4", "description": "", "selected": 1 } ] }
> }
> ```
>
> - Пять чекбоксов и два списка. Именно эти ключи выставляет наш `DeviceService.syncCapabilities`.
> - **`streamingType` — это выбор между `url.http` / `hls` / `hls2` / `hls4`**, которые приходят в
>   `videos[].files[].url`. У PWA выбран `4` (HLS4); мы играем `hls4` жёстко, не спрашивая. Если
>   когда-нибудь понадобится fallback на другой транспорт — переключатель живёт здесь, а не в
>   плеере.
> - `serverLocation` id: **1 = Нидерланды, 3 = Россия** (у 2 значения не видели). В ссылках это
>   отражается как `?loc=nl` в конце CDN-URL.
> - Странность: у веб-клиента `is_browser: false`, хотя это буквально браузер. Полагаться на флаг
>   не стоит.
> - Значения `description` приходят пустыми у всего, кроме HTTP.
